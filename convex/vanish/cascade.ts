...
async function sweepStorageFiles(
  ...
  if (user.avatarUrl && typeof user.avatarUrl === 'string') {
    ...
    console.log(`[VANISH STORAGE]   isStorageId: ${isStorageId}`);

    if (isStorageId) {
      try {
        console.log(`[VANISH STORAGE]   Attempting to delete storage file: ${user.avatarUrl}`);
      ...
      console.log(`[VANISH STORAGE]   Skipping HTTP URL: ${user.avatarUrl}`);
    }
  } else {
