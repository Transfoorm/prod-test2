...
export default function PasswordChangeCeremony({ onChangePassword }: PasswordChangeCeremonyProps) {
  ...
  // ─────────────────────────────────────────────────────────────────────
  return (
    <div ref={ceremonyRef} className="ft-password-ceremony" onKeyDown={handleKeyDown}>
      {/* BOX 1: Change Password */}
      <div
        ref={box1Ref}
        className={[
          'ft-password-ceremony__box',
          box1State.isIdle && 'ft-password-ceremony__box--idle',
          box1State.isValid && 'ft-password-ceremony__box--valid',
          box1State.hasError && 'ft-password-ceremony__box--error',
        ].filter(Boolean).join(' ')}
      >
        <label className="ft-password-ceremony__label">Change Password</label>
        <div className="ft-password-ceremony__input-wrapper">
          {box1State.isIdle ? (
            <div
              className="ft-password-ceremony__input ft-password-ceremony__input--masked"
              onClick={() => {
                setStage('typing');
            ...
            <input
              ...
              className="ft-password-ceremony__input"
              value={password}
          ...
          {box1State.isEditing && (
            <button
              type="button"
              className="ft-password-ceremony__eye"
          ...
          {!box1State.isEditing && (
            <button
              type="button"
              className="ft-password-ceremony__pill ft-password-ceremony__pill--empty"
        ...
        {passwordError ? (
          <div className="ft-password-ceremony__error">{passwordError}</div>
        ) : (
          <div className="ft-password-ceremony__helper">
            * At least 6 characters with 1 uppercase, 1 number, 1 symbol
          </div>
        )}
      </div>
      ...
      >
        <label className="ft-password-ceremony__label">Confirm Password</label>
        <div className="ft-password-ceremony__input-wrapper">
          {box2State.isEditing ? (
            <input
              ref={box2InputRef}
          ...
          {(passwordsMatch || stage === 'committing' || stage === 'success' || confirmError) && (
            ...
            >
              {stage === 'success' ? 'Changed ✓' :
               stage === 'committing' ? <span className="ft-password-ceremony__typing">Updating...</span> :
        ...
        {confirmError && (
          <div className="ft-password-ceremony__error">{confirmError}</div>
        )}
      </div>
    </div>
  );
