...
export default function ResetPasswordPage() {
  ...
  const [isSubmitting, setIsSubmitting] = useState(false); // FUSE: Optimistic state
  const [error, setError] = useState("");
  const firstCodeInputRef = useRef<HTMLInputElement>(null);

  // Define handleVerifyCodeSubmit before useEffects that use it
  const handleVerifyCodeSubmit = useCallback(async () => {
    ...
    setError("");
    setIsSubmitting(true); // FUSE: Optimistic
  ...
  }, [isLoaded, signIn, code, setStage, setError, setIsSubmitting]);

  // Auto-focus first code input when stage changes to code
  useEffect(() => {
  ...
  const handleSendCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log('SUCCESS Reset code request successful:', result);
      // Only proceed to code stage on success
      setStage("code");
      setIsSubmitting(false);
    } catch (err) {
      // Show Clerk's actual error message
  ...
  const handleResetPassword = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (result.status === "complete") {
        // Password reset successful - sign them in automatically
        if (result.createdSessionId) {
          ...
          // If no session created for some reason, show success and redirect to sign-in
          setStage("success");
          setTimeout(() => {
            router.push("/sign-in");
          }, 3000);
