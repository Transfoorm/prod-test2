...
export interface SortableColumn<TData = Record<string, unknown>> {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  onCheck?: (id: any) => void;  // Handler for row checkbox
  onHeaderCheck?: () => void;  // Handler for header checkbox
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  getRowId?: (row: TData) => any;  // Extract ID from row (defaults to row.id)
...
export interface SortableTableProps<TData = Record<string, unknown>> {
 ...
 */
function detectDateColumn<TData>(columns: SortableColumn<TData>[]): string | null {
  const datePatterns = [
    'createdAt',
    'dateDisplay',
    'created',
 ...
 */
export default function SortableTable<TData = Record<string, unknown>>({
  columns,
  data,
  striped = false,
  bordered = false,
            ...
            <tr key={rowIndex}>
              {columns.map((col) => {
                ...
                if (col.render) {
                  ...
                  const getRowId = col.getRowId || ((r: TData) => (r as Record<string, unknown>).id as string);
