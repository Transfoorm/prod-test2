  ...
  function visit(node: ts.Node) {
    ...
    if (
      ...
      if (
        ...
        if (tableInfo.userFields.length > 0 || tableInfo.indexes.length > 0) {
...
/**
 * Parse a defineTable call to extract user fields and indexes
 */
function parseTableDefinition(
  ...
  return { name: tableName, userFields, indexes };
}
